...
var get = async(Resumedb) => {
  var response;
  await Resumedb.find({}).then(card => response = card);
  return response;
}
...
module.exports = {
    ...
    const oldCards = await get(Resumedb);
    await newCards.map(async (newAux) => {
      const { type, layout, element, color } = newAux;
      if (newAux._id !== undefined) {
        oldCards.splice(oldCards.findIndex((old, index) => old._id == newAux._id), 1);
        await edit(Resumedb, { type, layout, element, color }, newAux._id)
      }
      else
        return await create(Resumedb, { type, layout, element, color });
    });
	...
	async fetch(req, res, next) {
		...
		res.send(await get(Resumedb));
  },
  save(req, res, next) {
    var db = Data.getDatabaseConnection(req.session.clinic.clinic_data);
    let Resumedb = db.model('savedCard', resumeSavedCards.savedCardSchema);
    Resumedb.create(req.body).then(card => res.send(card));
  },
  getSaved(req, res, next) {
    var db = Data.getDatabaseConnection(req.session.clinic.clinic_data);
    let Resumedb = db.model('savedCard', resumeSavedCards.savedCardSchema);
    Resumedb.find({type: req.body.type}).then(list => res.send(list)).catch(next);
  },
