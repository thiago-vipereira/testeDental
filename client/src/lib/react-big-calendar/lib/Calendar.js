...
var Calendar = function (_React$Component) {
  ...
  Calendar.prototype.render = function render() {
    var _props = this.props,
        ...
        props = _objectWithoutProperties(_props, ['toolbar', 'events', 'culture', 'components', 'formats', 'messages', 'style', 'className', 'elementProps', 'date', 'getNow', 'length']);
    ...
    if(!_props.patientSearch){
      label = View.title(current, { formats: formats, culture: culture, length: length });
    } else {
      if(events.length > 0){
        label = View.title(current, { formats: formats, culture: culture, length: length, end: events[events.length-1].start });
      }else{
        label = View.title(current, { formats: formats, culture: culture, length: length });
      }
    }
